...
def setup_logging_commands(subparsers):
    ...
    delete_parser.add_argument(
        "run_id", help="ID of the run to delete"
    )
    delete_parser.add_argument(
        "--log-dir", default="./logs",
        help="Logs directory"
    ...
    export_parser.add_argument(
        "run_id", help="ID of the run to export"
    )
    export_parser.add_argument(
    ...
    search_parser.add_argument(
        "--name", help="Run name to search for"
    )
    search_parser.add_argument(
        "--status", choices=["started", "completed", "error"],
        help="Run status to search for"
    )
    search_parser.add_argument(
        "--config-path", help="Config path to search for"
    )
    search_parser.add_argument(
        "--after", help="Show runs after this date (YYYY-MM-DD)"
    )
    search_parser.add_argument(
    ...
    # List metrics
    list_metrics_parser = metrics_subparsers.add_parser("list", help="List metrics for a run")
    list_metrics_parser.add_argument(
        "run_id", help="ID of the run"
    )
    list_metrics_parser.add_argument(
        "--format", choices=["text", "json"], default="text",
        help="Output format"
    ...
    dataset_metrics_parser = metrics_subparsers.add_parser("dataset", help="Show dataset metrics for a run")
    ...
    conversation_metrics_parser = metrics_subparsers.add_parser("conversation", help="Show metrics for a conversation")
    ...
    # Compare runs
    compare_parser = metrics_subparsers.add_parser("compare", help="Compare metrics between runs")
    compare_parser.add_argument(
        "run_ids", nargs="+", help="IDs of the runs to compare"
    )
    compare_parser.add_argument(
        "--format", choices=["text", "json"], default="text",
        help="Output format"
    ...
    if args.runs_command == "list":
        ...
        if args.after or args.before:
            ...
            for run in runs:
                if "timestamp" in run:
                    try:
                        run_date = datetime.datetime.fromisoformat(run["timestamp"]).date()
...
def handle_metrics_command(args):
    ...
    if args.metrics_command == "list":
        ...
        if args.format == "json":
            ...
            # Basic statistics
            lines.append("\nBasic statistics:")
            lines.append(f"  Conversations: {dataset_metrics.get('num_conversations', 0)}")
            lines.append(f"  Total messages: {dataset_metrics.get('total_messages', 0)}")
            lines.append(f"  Average messages per conversation: {dataset_metrics.get('avg_messages_per_conversation', 0):.2f}")
            ...
            # Speakers by role
            if "speakers_by_role" in dataset_metrics and dataset_metrics["speakers_by_role"]:
                lines.append("\nSpeakers by role:")
            ...
            # Entities
            if "entities" in dataset_metrics and "top_20" in dataset_metrics["entities"]:
            ...
            # Sentiments
            if "sentiments" in dataset_metrics and "frequency" in dataset_metrics["sentiments"]:
                lines.append(f"\nSentiments (total: {dataset_metrics['sentiments'].get('count', 0)}):")
                for sentiment, count in dataset_metrics["sentiments"]["frequency"].items():
                    lines.append(f"  {sentiment}: {count}")
            
            # Intents
            if "intents" in dataset_metrics and "frequency" in dataset_metrics["intents"]:
        ...
        metrics = logging_service.get_conversation_metrics(args.run_id, args.conversation_idx)
        ...
        if args.format == "json":
            ...
            lines = [f"Metrics for conversation {args.conversation_idx} in run {args.run_id}:"]
            ...
            lines.append(f"  Average message length: {metrics.get('avg_message_length', 0):.2f} characters")
            ...
            if "sentiment_distribution" in metrics and metrics["sentiment_distribution"]:
                lines.append("\n  Sentiment distribution:")
        ...
        if args.format == "json":
            ...
            for run_name in headers[1:]:
                row.append(f"{run_metrics[run_name].get('avg_messages_per_conversation', 0):.2f}")
            rows.append(row)
