...
class Message:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        for key in list(result.keys()):
            if result[key] is None or (isinstance(result[key], list) and len(result[key]) == 0):
                result[key] = None
...
class Conversation:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        result = {
            ...
            'speakers': {id: speaker.to_dict() for id, speaker in self.speakers.items()},
...
class Dataset:
    ...
    def export(
        ...
        if format == 'jsonl':
            ...
            dataset_name = os.path.basename(filepath).replace('.json', '').replace('.jsonl', '')
    ...
    def add_run_info(self, config_path: Optional[str] = None, config: Optional[Dict[str, Any]] = None) -> None:
    ...
    def from_file(cls, filepath: str) -> 'Dataset':
        ...
        if os.path.isdir(filepath):
            ...
            if not os.path.exists(metadata_path) or not os.path.exists(conversations_dir):
            ...
            if metadata.get('run_info'):
                ...
                if 'start_time' in metadata['run_info']:
                    run_info.start_time = datetime.fromisoformat(metadata['run_info']['start_time'])
                if 'end_time' in metadata['run_info'] and metadata['run_info']['end_time']:
                    run_info.end_time = datetime.fromisoformat(metadata['run_info']['end_time'])
            ...
            with open(filepath, 'r') as f:
                ...
                if data.get('run_info'):
                    ...
                    if 'start_time' in data['run_info']:
                        run_info.start_time = datetime.fromisoformat(data['run_info']['start_time'])
                    if 'end_time' in data['run_info'] and data['run_info']['end_time']:
                        run_info.end_time = datetime.fromisoformat(data['run_info']['end_time'])
                    dataset.run_info = run_info
