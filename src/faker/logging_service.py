...
class MetricsService:
    ...
    def compute_dataset_metrics(dataset: Dataset) -> Dict[str, Any]:
        ...
        for conv in dataset.conversations:
            ...
            for speaker in conv.speakers.values():
                ...
                speakers_by_role[speaker.role] = speakers_by_role.get(speaker.role, 0) + 1
        ...
        for conv in dataset.conversations:
            for msg in conv.messages:
                if msg.sentiment:
                    sentiment_frequency[msg.sentiment] = sentiment_frequency.get(msg.sentiment, 0) + 1
                if msg.intent:
                    intent_frequency[msg.intent] = intent_frequency.get(msg.intent, 0) + 1
                if msg.topics:
        ...
        metrics = {
            ...
            "topics": {
                ...
                "top_20": dict(sorted(topic_frequency.items(), key=lambda x: x[1], reverse=True)[:20])
            },
            "entities": {
                "count": len(all_entities),
                "top_20": dict(sorted(entity_frequency.items(), key=lambda x: x[1], reverse=True)[:20])
            },
            "sentiments": {
    ...
    def compute_conversation_metrics(conversation: Conversation) -> Dict[str, Any]:
        ...
        for msg in conversation.messages:
            messages_by_speaker[msg.speaker_id] = messages_by_speaker.get(msg.speaker_id, 0) + 1
        
        # Map speaker IDs to roles
        speaker_roles = {id: speaker.role for id, speaker in conversation.speakers.items()}
...
class LoggingService:
    ...
    def complete_run(self, run_id: str, dataset: Dataset) -> None:
        ...
        run_info["duration"] = (datetime.fromisoformat(run_info["end_time"]) - 
                               datetime.fromisoformat(run_info["start_time"])).total_seconds()
        ...
        for i, conversation in enumerate(dataset.conversations):
            conv_metrics = self.metrics_service.compute_conversation_metrics(conversation)
        ...
        self.logger.info(f"Completed run {run_id}, duration: {run_info['duration']:.2f}s")
    ...
    def export_run_report(self, run_id: str, format: str = "json") -> str:
        ...
        if format == "json":
            ...
            if "dataset_metrics" in metrics:
                ...
                lines.append(f"  Avg Messages/Conversation: {dataset_metrics.get('avg_messages_per_conversation', 0):.2f}")
                
                if "message_length" in dataset_metrics:
                    msg_len = dataset_metrics["message_length"]
                    lines.append(f"  Message Length (avg): {msg_len.get('avg', 0):.2f} chars")
                    lines.append(f"  Message Length (min): {msg_len.get('min', 0)} chars")
                    lines.append(f"  Message Length (max): {msg_len.get('max', 0)} chars")
    ...
    def get_conversation_metrics(self, run_id: str, conversation_idx: int) -> Optional[Dict[str, Any]]:
    ...
    def save_custom_metric(self, run_id: str, metric_name: str, metric_value: Any) -> None:
