...
def parse_args():
    ...
    generate_parser.add_argument("--log-file", help="Log file path")
    generate_parser.add_argument(
        "--log-dir", default="./logs", help="Directory for storing run logs"
    )
    generate_parser.add_argument(
        "--use-db", action="store_true", help="Use SQLite database for run logs"
    )
    generate_parser.add_argument(
        "--no-export-run-info", action="store_true", help="Don't export run information"
    )
    generate_parser.add_argument(
        "--no-logging-service",
        action="store_true",
    ...
    view_parser.add_argument("file", help="Path to dataset file or directory")
    view_parser.add_argument(
        "-i", "--index", type=int, help="Index of conversation to view"
    )
    view_parser.add_argument("--id", help="ID of conversation to view")
    view_parser.add_argument(
    ...
    info_parser = subparsers.add_parser(
        "info", help="Show information about the system"
    )
...
def format_dataset_stats(dataset):
    ...
    if dataset.run_info:
        ...
        lines.append(
            f"  End Time: {dataset.run_info.end_time if dataset.run_info.end_time else 'N/A'}"
        )
    ...
    if args.command == "generate":
        ...
        try:
            ...
            if num_conversations is None:
                ...
                num_conversations = generator.config.get("dataset", {}).get(
                    "num_conversations", 1
                )
            ...
            if logging_service and dataset.run_info:
                ...
                print(
                    f"View run details with: python -m faker runs show {dataset.run_info.id}"
                )
                print(
                    f"View dataset metrics with: python -m faker metrics dataset {dataset.run_info.id}"
                )
        ...
        try:
            ...
            if args.metadata_only:
                ...
                metadata = {
                    ...
                    "run_info": (
                        dataset.run_info.to_dict() if dataset.run_info else None
                    ),
                    "num_conversations": len(dataset.conversations),
                }
            ...
            if args.id:
                conversation = next(
                    (c for c in dataset.conversations if c.id == args.id), None
                )
                if conversation:
            ...
            # Show summary of all conversations
                ...
                for i, conv in enumerate(dataset.conversations):
                    ...
                    print(
                        f"{i}: ID {conv.id} ({num_messages} messages, {speakers} speakers{domain})"
                    )
        ...
        if args.list_configs:
            ...
            if configs:
                ...
                print(
                    "You can create a configuration file or copy an example from the documentation."
                )
        ...
        logger.error(
            "No command specified. Run 'python -m faker --help' for usage information."
        )
        return 1
